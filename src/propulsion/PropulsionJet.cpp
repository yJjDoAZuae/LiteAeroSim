...
void PropulsionJet::onInitialize(const nlohmann::json& config) {
    _thrust_sl_n    = config.at("thrust_sl_n").get<float>();
    _bypass_ratio   = config.at("bypass_ratio").get<float>();
    _inlet_area_m2  = config.at("inlet_area_m2").get<float>();
    _idle_fraction  = config.at("idle_fraction").get<float>();
    _spool_tau_s    = config.at("spool_tau_s").get<float>();
    _ab_thrust_sl_n = config.at("ab_thrust_sl_n").get<float>();
    _ab_spool_tau_s = config.at("ab_spool_tau_s").get<float>();
    const float dt_s = config.at("dt_s").get<float>();

    _density_exponent = 1.f / std::sqrt(1.f + _bypass_ratio);

    _spool_filter.setLowPassFirstIIR(dt_s, _spool_tau_s);
    _spool_filter.valLimit.enableLower();
    _spool_filter.valLimit.enableUpper();

    const float ab_tau = _ab_spool_tau_s > 0.f ? _ab_spool_tau_s : _spool_tau_s;
...
void PropulsionJet::onReset() {
    _spool_filter.resetToOutput(0.f);
    _ab_filter.resetToOutput(0.f);
    _thrust_n  = 0.f;
    _ab_active = false;
}

nlohmann::json PropulsionJet::onSerializeJson() const {
...
void PropulsionJet::onDeserializeJson(const nlohmann::json& j) {
    if (j.at("type").get<std::string>() != "PropulsionJet")
        throw std::runtime_error("PropulsionJet::onDeserializeJson: type mismatch");
...
float PropulsionJet::step(float throttle_nd, float tas_mps, float rho_kgm3) {
    ...
    // ── Afterburner ──
    const float T_ab_avail  = _ab_thrust_sl_n
                              * std::pow(rho_kgm3 / kRhoSL_kgm3, _density_exponent);
