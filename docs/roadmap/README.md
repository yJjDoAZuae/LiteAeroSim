...
| --- | --- | --- | --- |
| Simulation plant — aircraft physics, environment, terrain, sensors, external interfaces, sim runner | LiteAeroSim | [aircraft.md](aircraft.md) | Active |
| Flight code — autopilot, guidance, path, navigation, estimation, gain scheduling | FlightCode | [flight_code.md](flight_code.md) | Planned — `flightcode` repo not yet created |
...
| **`flightcode` repository creation** — create repo; set up CMake build system with one target per subsystem (`avraero::log`, `avraero::control`, `avraero::terrain`, `avraero::nav`, `avraero::guidance`, `avraero::autopilot`, `avraero::perception`, `avraero::mission_autonomy`); migrate shared interface types | FlightCode development | Planned |
| **Repo split and namespace migration** — migrate `DynamicElement`/`SisoElement`/Filter hierarchy → `avraero::control`; `ILogger` → `avraero::log`; terrain mesh types / `V_Terrain` → `avraero::terrain`; shared interface types → `flightcode`; apply `avraero::` namespace to all code; update LiteAeroSim to take a versioned dependency on `flightcode` | FlightCode development; SITL verification pipeline | Planned — deferred to repo split milestone; all target namespaces are final |

---

## Integration and Operations

Items that span both LiteAeroSim and FlightCode, or that represent end-to-end capability
milestones. Detailed design for each item will be documented before implementation begins.

| # | Item | Depends on |
| --- | --- | --- |
| I-1 | **Containerized SITL (UC-2b)** — `liteaerosim` and `flightcode` Docker containers on the same host; ICD-8 network transport; QGroundControl connection; demonstrated closed-loop flight | Repo split; SimRunner (LAS-13); FlightCode autopilot + guidance + navigation; `ArduPilotInterface` or `PX4Interface` or LiteAeroSim Autopilot |
...
| I-6 | **HITL — Unmodified ArduPilot + companion computer (UC-3a)** — LiteAeroSim on desktop; ArduPilot autopilot board; companion computer running FlightCode navigation and guidance components over MAVLink | I-2; FlightCode navigation + guidance |
| I-7 | **HITL — Unmodified PX4 + companion computer (UC-3b)** — same structure as I-6 with PX4 replacing ArduPilot | I-3; FlightCode navigation + guidance |
