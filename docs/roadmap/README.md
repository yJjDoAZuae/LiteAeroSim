...
| **`liteaero-flight` repository creation** — create repo; set up CMake build system with one target per subsystem (`avraero::log`, `avraero::control`, `avraero::terrain`, `avraero::nav`, `avraero::guidance`, `avraero::autopilot`, `avraero::perception`, `avraero::mission_autonomy`); migrate shared interface types | LiteAero Flight development | Planned |
| **Repo split and namespace migration** — migrate `DynamicElement`/`SisoElement`/Filter hierarchy → `avraero::control`; `ILogger` → `avraero::log`; terrain mesh types / `V_Terrain` → `avraero::terrain`; shared interface types → `liteaero-flight`; apply `avraero::` namespace to all code; update LiteAero Sim to take a versioned dependency on `liteaero-flight` | LiteAero Flight development; SITL verification pipeline | Planned — deferred to repo split milestone; all target namespaces are final |
